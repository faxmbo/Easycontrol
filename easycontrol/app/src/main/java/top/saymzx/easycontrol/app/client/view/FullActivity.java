...
public class FullActivity extends Activity implements SensorEventListener {
    ...
    private boolean lastIsLandscape=false;
    @Override
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        boolean isLandscape = clientController.vwidth > clientController.vheight;
        final float THRESHOLD = 4.5f;

        // 当横竖屏状态变化时，设置默认方向
        if (isLandscape != lastIsLandscape) {
            lastOrientation = -1; // 强制触发方向更新
            // 设置默认方向（横屏默认正横屏，竖屏默认正竖屏）
            newOrientation = isLandscape ?
        ...
        // 原有方向判断逻辑
            ...
            if (isLandscape) {
                if (x <= -THRESHOLD) newOrientation = ActivityInfo.SCREEN_ORIENTATION_REVERSE_LANDSCAPE;
                else if (x >= THRESHOLD)newOrientation = ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE;

            } else {
                float y = values[1];
                if (y <= -THRESHOLD) newOrientation = ActivityInfo.SCREEN_ORIENTATION_REVERSE_PORTRAIT;
                else if (y >= THRESHOLD) newOrientation = ActivityInfo.SCREEN_ORIENTATION_PORTRAIT;

            }
        }

        if (lastOrientation != newOrientation && newOrientation != -1) {
